...
router.get('/cart', function(req, res, next) {
    if (!Array.isArray(req.session.cart) || !req.session.cart.length){
        ...
        let query = "SELECT product_id, product_type, product_size, product_material, product_condition, product_brand, product_price FROM product WHERE product_id IN (" + req.session.cart + ") order by find_in_set(product_id, '" + req.session.cart + "');";
        // execute query
        db.query(query, (err, result) => {
            if (err) {
                res.render('error');
